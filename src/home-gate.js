...
async function renderList(e,c,page,admin=false,messageId=null){const size=15;const count=await e.DB.prepare("SELECT COUNT(*) n FROM tracks").first();const total=Math.max(1,Math.ceil(Number(count?.n||0)/size));const p=Math.min(Math.max(Number(page)||1,total?1:1),total);const rows=(await e.DB.prepare("SELECT id,url,title FROM tracks ORDER BY id DESC LIMIT ? OFFSET ?").bind(size,(p-1)*size).all()).results||[];if(!Number(count?.n||0))return tg(e,"sendMessage",{chat_id:c,text:"📜 <b>فهرست پست‌های ثبت‌شده</b>\n\nهنوز هیچ پستی ثبت نشده است.",parse_mode:"HTML",...(admin?{reply_markup:listKeyboard(1,1,true)}:{})});const body={chat_id:c,text:listText(rows,p,total),parse_mode:"HTML",reply_markup:listKeyboard(p,total,admin)};return messageId?tg(e,"editMessageText",{...body,message_id:messageId}):tg(e,"sendMessage",body)}
