...
async function ensureCompat(e){
  ...
  })().catch(err=>{compatPromise=null;throw err});
...
export default{async fetch(req,e){let u;try{u=await req.clone().json()}catch{return import("./publish-router.js").then(m=>m.default.fetch(req,e))}const q=u.callback_query;if(q){const d=String(q.data||"");if(d==="help"){await tg(e,"answerCallbackQuery",{callback_query_id:q.id});return edit(e,q,guide,{inline_keyboard:[[{text:"🔙 منوی اصلی",callback_data:"public_start"}]]})}if(d==="public_start"){await tg(e,"answerCallbackQuery",{callback_query_id:q.id});return edit(e,q,startText,startKeyboard)}if((d==="stats"||d==="stats_inline")&&admin(e,q.from?.id)){await tg(e,"answerCallbackQuery",{callback_query_id:q.id});return stats(e,q)}if(d==="pickled_add"&&admin(e,q.from?.id)){await tg(e,"answerCallbackQuery",{callback_query_id:q.id});return import("./pickle-master.js").then(m=>m.default.fetch(req,e))}}return import("./publish-router.js").then(m=>m.default.fetch(req,e))}};