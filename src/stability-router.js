...
async function ensureCompat(e){
  ...
  const add=async(table,column,type)=>{try{const x=await e.DB.prepare(`PRAGMA table_info(${table})`).all();if(!(x.results||[]).some(r=>r.name===column))await e.DB.prepare(`ALTER TABLE ${table} ADD COLUMN ${column} ${type}`).run()}catch{}};
  await add("sessions","control_message_id","INTEGER");
  await add("sessions","draft_parts","TEXT");
  await add("sessions","draft_title","TEXT");
  await add("tracks","display_mode","TEXT");
...
export default{async fetch(req,e){await ensureCompat(e);let u;try{u=await req.clone().json()}catch{return import("./publish-router.js").then(m=>m.default.fetch(req,e))}const q=u.callback_query;if(q&&admin(e,q.from?.id)){const d=String(q.data||"");if(d==="help"){await tg(e,"answerCallbackQuery",{callback_query_id:q.id});return edit(e,q,guide,{inline_keyboard:[[{text:"🔙 منوی اصلی",callback_data:"public_start"}]]})}if(d==="stats"||d==="stats_inline"){await tg(e,"answerCallbackQuery",{callback_query_id:q.id});return stats(e,q)}}return import("./publish-router.js").then(m=>m.default.fetch(req,e))}};