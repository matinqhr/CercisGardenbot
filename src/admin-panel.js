...
async function stats(e,q){
  ...
  const sessions=await n(e,"SELECT COUNT(*) n FROM sessions WHERE step IS NOT NULL AND TRIM(step)!=''");
  ...
  const requests=await n(e,"SELECT COALESCE(SUM(count),0) n FROM usage_stats WHERE LOWER(key) LIKE '%request%'");
